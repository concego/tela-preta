document.addEventListener('DOMContentLoaded', () => {
    ...
    const clearFileButton = document.getElementById('clear-file-button'); // Botão Limpar Arquivo
    const saveFileButton = document.getElementById('save-file-button'); // NOVO: Botão Salvar Arquivo
    ...
    function createNewProject() {
        ...
        if (projectName) {
            ...
            // MODIFICADO: Cria projeto com o objeto 'files' vazio
        ...
        const appendTree = (parentEl, node, currentPath = '') => {
            Object.keys(node).forEach(name => {
                ...
                if (typeof node[name] === 'object') { // É uma pasta
                    ...
                    // NOVO: Adiciona classe 'active-file-tree' para o arquivo ativo
                    if (node[name] === activeTabPath) { // Compara com o caminho da aba ativa
                        li.classList.add('active-file-tree');
                    }
                }
    ...
    function createNewFileOrFolder() {
        ...
        // Remove barras iniciais ou múltiplas barras consecutivas
        newPath = newPath.replace(/^\/|\/\/+/g, '/').replace(/^\//, ''); // Remove '/' do início e substitui '//' por '/'
        ...
        if (newPath.endsWith('/')) { // É uma pasta
            ...
            project.files[newPath] = { content: '', type: 'folder' };
            saveProjectsToLocalStorage();
            renderFileTree();
    ...
    function renameFileInProject() {
        ...
        newPath = newPath.trim();
        newPath = newPath.replace(/^\/|\/\/+/g, '/').replace(/^\//, ''); // Limpa o path de novo

        if (newPath === oldPath) return; // Sem alteração

        if (project.files[newPath] && newPath !== oldPath) { // Verifica se o novo nome já existe e não é o mesmo
        ...
        // Renomear todos os arquivos/pastas que começam com o oldPath
        const filesToRename = Object.keys(project.files).filter(filePath => filePath.startsWith(oldPath));
        const newFiles = {};

        filesToRename.forEach(oldFilePath => {
            const newFilePath = newPath + oldFilePath.substring(oldPath.length);
            newFiles[newFilePath] = project.files[oldFilePath];
            delete project.files[oldFilePath]; // Deleta a entrada antiga
        });

        // Adiciona os arquivos renomeados de volta ao projeto
        Object.assign(project.files, newFiles);
    ...
    function deleteFileInProject() {
        if (!currentProjectName || (!activeTabPath && !fileTreeElement.querySelector('li.active-file-tree'))) {
            alert("Selecione um arquivo ou pasta na árvore ou abra um arquivo na aba para excluir.");
            return;
        }
        const project = allProjectsData[currentProjectName];
        ...
        // Se nenhuma aba ativa, tenta pegar da seleção na árvore
        if (!filePathToDelete) {
            const selectedFileElement = fileTreeElement.querySelector('li.active-file-tree');
            if (selectedFileElement && selectedFileElement.dataset.filePath) {
                filePathToDelete = selectedFileElement.dataset.filePath;
            } else {
                 alert("Por favor, selecione um arquivo ou pasta na árvore de arquivos.");
                 return;
            }
        }

        // Verifica se o path realmente existe ou se é um prefixo de pasta
        const isFolderToDelete = filePathToDelete.endsWith('/');
        const itemExists = project.files[filePathToDelete] || Object.keys(project.files).some(p => p.startsWith(filePathToDelete + '/'));

        if (!filePathToDelete || !itemExists) {
    ...
    /**
     * Limpa o conteúdo do arquivo atualmente aberto no editor.
     */
    function clearCurrentFile() {
        if (!currentProjectName || !activeTabPath) {
            alert('Nenhum arquivo está aberto para ser limpo.');
            return;
        }

        if (confirm(`Tem certeza que deseja limpar o conteúdo de "${activeTabPath}"? Esta ação não pode ser desfeita.`)) {
            editor.setValue(''); // Define o valor do editor para uma string vazia
            openTabs[activeTabPath].content = ''; // Limpa o conteúdo na aba aberta
            allProjectsData[currentProjectName].files[activeTabPath].content = ''; // Atualiza o conteúdo no objeto de dados do projeto
            saveProjectsToLocalStorage(); // Salva as alterações
            logToCustomConsole(`O conteúdo de "${activeTabPath}" foi limpo.`, "info");
        }
    }

    /**
     * Salva o conteúdo do arquivo atualmente aberto no editor.
     */
    function saveCurrentFile() {
        if (!currentProjectName || !activeTabPath || !editor) {
        ...
        // Atualiza o conteúdo no objeto de abas abertas
        openTabs[activeTabPath].content = contentToSave;

        // Atualiza o conteúdo no objeto de dados do projeto (onde o arquivo realmente vive)
        allProjectsData[currentProjectName].files[activeTabPath].content = contentToSave;

        saveProjectsToLocalStorage(); // Salva os dados no Local Storage
        logToCustomConsole(`Arquivo "${activeTabPath}" salvo manualmente.`, "info");
    }
    ...
    function renderEditorTabs() {
        ...
        if (Object.keys(openTabs).length === 0) {
            ...
            clearFileButton.disabled = true; // Desabilita o botão de limpar
            saveFileButton.disabled = true; // Desabilita o botão de salvar
            return;
        } else {
            codeEditorTextarea.style.display = 'block'; // Garante que o textarea esteja visível
            clearFileButton.disabled = false; // Habilita o botão de limpar
            saveFileButton.disabled = false; // Habilita o botão de salvar
        }
        ...
        Object.keys(openTabs).forEach(filePath => {
            ...
            tabElement.setAttribute('tabindex', '0'); // Torna a aba focável
            ...
            tabElement.addEventListener('keydown', (e) => { // Acessibilidade: navegar abas com teclado
    ...
    function switchTab(filePath) {
        ...
        if (currentProjectName && allProjectsData[currentProjectName]) {
             ...
             saveProjectsToLocalStorage();
             renderFileTree(); // Atualiza a árvore de arquivos para destacar o item ativo
        }
    ...
    function closeTab(filePath) {
        ...
        if (filePath === activeTabPath && editor) {
        ...
        if (filePath === activeTabPath) {
            ...
            if (remainingPaths.length > 0) {
                ...
                if (!editor) {
                    editor = createEditorInstanceForTab(codeEditorTextarea, 'text/plain'); // Modo de texto simples
                }
                editor.setValue(`Bem-vindo ao Tela Preta IDE!

Para começar:
1. Clique em "Novo Projeto" na barra lateral para criar um novo projeto.
2. Com o projeto criado, clique em "Novo Arquivo/Pasta" para adicionar arquivos como 'index.html', 'style.css', 'script.js', etc.
3. Clique em um arquivo na árvore de arquivos para abri-lo no editor.
4. Digite seu código e veja o resultado na aba "Preview" ou os logs na aba "Console".`);
            }
        }
    ...
    function showOutputPanel(panelId) {
        ...
        if (panelId === 'preview-iframe') {
            ...
            previewTabButton.setAttribute('aria-selected', 'true');
            consoleTabButton.setAttribute('aria-selected', 'false');
        } else {
            consoleTabButton.classList.add('active');
            consoleTabButton.setAttribute('aria-selected', 'true');
            previewTabButton.setAttribute('aria-selected', 'false');
        }
    }
    ...
    function runCode() {
        ...
        if (!files[mainHtmlFile] || files[mainHtmlFile].type !== 'html') {
             // Busca o primeiro arquivo .html se 'index.html' não existir ou não for HTML
            mainHtmlFile = Object.keys(files).find(filePath =>
                (filePath.endsWith('.html') || filePath.endsWith('.htm')) && files[filePath].type === 'html'
            );
        }
        ...
        Object.keys(files).forEach(filePath => {
            ...
            // Ignora o HTML principal se já o pegamos e evita pastas
            if (filePath === mainHtmlFile || file.type === 'folder') return;
    ...
    function initializeApp() {
        ...
        if (Object.keys(allProjectsData).length === 0) {
            ...
            currentProjectNameSpan.textContent = 'Nenhum Projeto Aberto';
...
4. Digite seu código e veja o resultado na aba "Preview" ou os logs na aba "Console".`);
    ...
    saveFileButton.addEventListener('click', saveCurrentFile); // Listener para o botão de salvar
